...
raw_data <- read.table("~/Dropbox/02-OweniaGenome/06-Revision/00-DATA/05-Phylostratigraphy_with_original_species_Owenia_and_Capitella/03-Dimorphilus_gyrociliatus/01-Raw_data_Dimorphilus_gyrociliatus.txt", 
                       as.is=TRUE, header=TRUE)
rownames(raw_data) <- t(raw_data[c(1)])
raw_data_clean <- raw_data[-c(1,2)]
colnames(raw_data_clean)[1] <- "origin"
...
norm$origin[norm$origin == "Dgyr-2"] <- "Dgyr"
norm$origin[norm$origin == "Nephrozoa" | norm$origin == "Bilateria"] <- "Nephrozoa_Bilateria"

colnames(norm) <- c("origin","early_development", "late_development", 
                    "hatchling", "female_adult")
norm_tidy <- gather(norm, stage, expr, -origin)
summary(norm)

dgyr_norm <- norm_tidy %>% filter(grepl('Dgyr', origin))
...
dgyr_norm$stage <- factor(dgyr_norm$stage, levels = unique(dgyr_norm$stage))
...
norm_75_quant <- data.frame(tapply(norm$early_development, norm$origin, quantile, p = 0.75, na.rm = TRUE),
                            tapply(norm$late_development, norm$origin, quantile, p = 0.75, na.rm = TRUE),
                            tapply(norm$hatchling, norm$origin, quantile, p = 0.75, na.rm = TRUE),
                            tapply(norm$female_adult, norm$origin, quantile, p = 0.75, na.rm = TRUE))
colnames(norm_75_quant) <- c("early_development", "late_development", 
                             "hatchling", "female_adult")
rownames(norm_75_quant) <- c("Annelida","Dimorphilus_gyrociliatus","Lophotrochozoa","Metazoa",
...
dimorphilus_gyrociliatus_plot <- ggplot(dgyr_norm, aes(x = stage, y = expr)) +
  geom_boxplot(aes(x = stage, y = expr, fill = stage), outlier.shape = NA,
               width = 0.7) +
  scale_fill_manual(values = c("#563B45","#D2A359","#C6613A",
                               "#0B6CA0","#032C65","#507810",
...
ggarrange(metazoa_plot, nephrozoa_bilateria_plot, protostomia_plot,
          spiralia_plot, annelida_plot, dimorphilus_gyrociliatus_plot, 
...
clusters <- read.table("/Users/franciscomanuelmartinzamora/Dropbox/02-OweniaGenome/06-Revision/00-DATA/07-Dimorphilus_RNAseq_core_analyses/06-Dimorphilus_gyrociliatus_clusters_annotation_corrected.txt",
...
lineages_expr$origin[lineages_expr$origin == "Dgyr-2"] <- "Dimorphilus_gyrociliatus"
lineages$origin[lineages$origin == "Dgyr"] <- "Dimorphilus_gyrociliatus"
...
pvalues <- data.frame(Metazoa=numeric(), Nephrozoa_Bilateria=numeric(), Protostomia=numeric(),
                      Lophotrochozoa=numeric(), Annelida=numeric(), Dimorphilus_gyrociliatus=numeric(), Other=numeric())
oddsratio <- data.frame(Metazoa=numeric(), Nephrozoa_Bilateria=numeric(), Protostomia=numeric(),
                        Lophotrochozoa=numeric(), Annelida=numeric(), Dimorphilus_gyrociliatus=numeric(), Other=numeric())

for (i in c("Metazoa","Nephrozoa_Bilateria","Protostomia","Lophotrochozoa","Annelida","Dimorphilus_gyrociliatus","Other")){
  for (j in c(1:9)){
...
pvalues$Dimorphilus_gyrociliatus <- p.adjust(pvalues$Dimorphilus_gyrociliatus, method = "bonferroni")
...
count_table["Dimorphilus_gyrociliatus",] <- c(count$Freq[count$Var1 == "Dgyr"])
count_table["Other",] <- c(count$Freq[count$Var1 == "others"])
count_table <- round(count_table)

write.table(count_table[1], "03-Dimorphilus_gyrociliatus/07-Phylostrata_gene_count.txt", sep ='\t', quote = F, col.names = F)
